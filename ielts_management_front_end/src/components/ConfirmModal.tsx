...
export default function ConfirmModal({
    ...
    return createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
            ...
            {/* Modal */}
            <div
                className={`relative w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-2xl transition-all duration-300 ${isVisible ? "scale-100 opacity-100 translate-y-0" : "scale-95 opacity-0 translate-y-4 sm:translate-y-0"
                    }`}
            >
                <button
                    onClick={!isLoading ? onClose : undefined}
                ...
                <div className="flex items-start gap-4">
                    <div
                        className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full ${isDestructive ? "bg-red-100" : "bg-blue-100"
                            }`}
                    >
                        <AlertTriangle
                ...
                <div className="mt-8 flex items-center justify-end gap-3">
                    ...
                    <button
                        ...
                        onClick={onConfirm}
                        disabled={isLoading}
                        className={`flex items-center justify-center min-w-[100px] rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition-colors disabled:opacity-50 ${isDestructive
